...
\begin{document}
    ...
    Note that every perfect matching is a \tit{maximal matching}. Additionally, note that a perfect matching exists only if the number of vertices of $G$ is even, since each edge of the perfect matching covers exactly 2 vertices.
    ...
    \begin{proof}
        Consider a min-CDS $D$ for $G$; a ST $T'$ for $G$ can be constructed by starting from $D$, by simply considering $D$ itself with the nodes in $V(G) - D$ as leaves of $T'$. Note that $D$ cannot contain cycles, because $D$ is a min-CDS, and a CDS with cycles would not have the minimum possible cardinality because removing one vertex from a cycle would still yield a CDS, of lower cardinality. Moreover, $T'$ spans $G$ completely, because $D$ is a DS, which means that all the nodes in $V(G) - D$ are covered by definition of $D$. This proves that $T'$ is both connected, acyclic, and spans $G$ completely, therefore it is a spanning tree of $G$, and because $T$ is a max-leaf ST for $G$, it follows that $$l \ge l' = \abs{V(G)} - \abs D = n - d$$ where $l'$ is the number of leaves of $T'$, which is $n - d$ by construction.
    ...
    \begin{framedalgo}{Two-step greedy min-CDS}
        ...
        \begin{algorithmic}[1]
            \Function{greedyMinCDS}{$G$, B, Gr, W}
                \State $M := \varnothing$
                \If{there exists a node $v \in \mathrm W \cup \mathrm{Gr}$ such that coloring it in black, and its adjacent white nodes in gray, reduces the value of $\abs{\mathrm W} + \mathrm{CC}$}
                    \State $M = M \cup \{v\}$
                \EndIf
                \Do
