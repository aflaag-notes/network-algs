...
\begin{document}
    ...
    Moreover, all known algorithm for finding the least cost path between any two nodes on a graph are based on \tbf{graph exploration}, which is based on multiple \tit{walks} (i.e. paths where vertices may be repeated) on the graph. This raises a problem when there are \tbf{negative-weight cycles}, because the walk could take such a cycle infinitely many times, and the weight of the path between the two nodes can be lowered infinitely, without halting. Therefore, only networks without negative-weight cycles will be discussed.
    ...
    \begin{itemize}
        ...
        \item cycles having \tbf{null length} cannot exist, by assumption: if there is such a cycle in a solution, then a solution withouth the cycle would yield a path of the same weight;
    \end{itemize}
    ...
    The Bellman-Ford algorithm is used for \href{https://en.wikipedia.org/wiki/Distance-vector_routing_protocol}{distance vector routing protocol}, an iterative, asynchronous and distributed protocol.
    ...
    \idea{
        ...
        \begin{itemize}
            ...
            \item for each outcoming edge $(u, v)$ from $u$, $v$ is relaxed w.r.t $(u, v)$, and $Q$ is updated based on $d(v)$.
        \end{itemize}
    }
    ...
    \begin{example}[Vertex covers]
        The following are two examples of minimal vertex covers for the given graph:
        \begin{figure}[H]
            \centering
    ...
    \cost{
        Edge has to be explored or removed by the algorithm, therefore the cost is $O(n + m)$.
    }
    ...
    \cost{
        The cost of the algorithm is the same of the \cref{alg:first_greedy_avc}, which is $O(n +m)$.
    }
    ...
    \begin{proof}
        Let $V'$ be the set of vertices described by $y^*$. It is easy to see that $V'$ is a vertex cover for $G$, because the constraint of the ILP $$x^*_i + x^*_j \ge 1$$ forces at least one between $x^*_i$ and $x^*_j$ to be greater or equal than $\frac{1}{2}$, therefore at least one between $y^*_i$ and $y_i^*$ will be set to 1.
    ...
    \proofiff{
        By way of contradiction, assume that there exist $x, y \in V(G) - V'$ such that $(x, y) \in E(G)$; then, neither $x$ nor $y$ are in $V'$, therefore $(x, y)$ is not covered by the vertex cover $V'$.
    }{
    ...
    \begin{proof}
        Consider an edge $(u, v) \in E(G)$; this edge is either in $M$, and therefore both endpoints are inserted into $V'$ by the algorithm, or $(u, v) \in E(G) - M$, but at least one of its endpoints must be in $V'$, otherwise it could have been upt into $M$, but this is not possible because $M$ is maximal. This proves that $V'$ is a vertex cover.
    ...
    \begin{frameddefn}{Bipartite graph}
        A graph $G$ is said to be \tbf{bipartite} if and only if there is a partition $U, W$ of $V(G)$ such that both $U$ and $V$ are independent set.
    \end{frameddefn}

    Bipartite graphs are very important, because the following theorem, proved by König \cite{konig}, allows to compute a VC on bipartite graphs in polynomial time.
    ...
    Note that $V'$ can induce at most $\abs{V'}$ connected components, we get the following theorem.
    ...
    \begin{proof}
        Consider a vertex cover of less than $n - 1$ nodes of $P_n$. It is always possible to desing an attack strategy such that all the guard from a connected path, therefore an edge will be unprotected because there are less than $n - 1$ guards.
    \end{proof}
