...
\begin{document}
    ...
    Each station can dynamically \tit{modulate} its own transmission power, in fact the \tbf{transmission radius} of a station depends on the energy power supplied to the station. In particular, the power $P_s$ required by a station $s$ to transmit dta to another station $t$ must satisfy the following inequality $$\dfrac{P_s}{\dist^\alpha(s, t) } \ge 1$$ where $\alpha \ge 1$ is called \tbf{distance-power gradient}. Usually $2 \le \alpha \le 4$, and $\alpha = 2$ in empty space. This implies that, in order to have a communication from $s$ to $t$, the power $P_s$ must be \tit{proportional} to $\dist^\alpha(s, t)$. 

    % The primary goal is to minimize energy consumption, as all devices rely on a common electricity source (e.g., when the stations are deployed in an ad-hoc fashion and connected to a centralized power supply).
