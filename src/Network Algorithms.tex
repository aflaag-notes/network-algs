...
\begin{document}
    ...
    \subsection{Routing on a butterfly network}
    ...
    \begin{example}[Beneš network]
        The following is an example of a Beneš network. \todo{add pic}
    \end{example}
    ...
    \proofind{
        ...
        Given any one-to-one mapping $\pi$ of the $2^n$ inputs and outputs on a $(n - 1)$-dimensional Beneš network, there exists \tit{a set of edge-disjoint paths} from the inputs to the outputs connecting each input $i$ to output $\pi(i)$, for each $1 \le i \le 2^{n}$.
    }{
        TODO
    }
