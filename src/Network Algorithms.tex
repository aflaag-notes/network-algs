...
\begin{document}
    ...
    \chapter{UAVs monitoring problem}
    ...
    Let's try to find a problem which satisfies all the constraints of the \tbf{UAVs monitoring problem}.
    ...
    This problem does not model the UAVs monitoring problem appropriately because it has no visiting time nor \curlyquotes{battery capacity} constraint.
    ...
    This problem is a bit more similar to our problem, but it still does not model the UAVs monitoring problem appropriately because it has no battery constraint.
    ...
    No previously studied problem seems to model the UAVs monitoring problem appropriately, which implies that we cannot exploit any know result.

    \section{Connection with the RMCCP}
    
    Given a set of locations $V$, a \tbf{cycle cover} $\mathcal C := \{C_1, \ldots, C_k\}$ for $V$ is a set of cycles such that each location in $V$ belongs to at least one cycle of $\mathcal C$. The \tbf{completion time} of a cycle cover is defined as the maximum \tit{cost} among all the cycles of the cycle cover.
    ...
    placeholder \todo{slide 21 papers}

    It can be shown that the RMCCP and the UAVs monitoring problem are \tit{tightly connected}.

    \begin{framedthm}{}
        If the RMCCP can be approximated with an approximation ratio of $\alpha$, the UAVs monitoring problem can be approximated with an approximation ratio of $5 \alpha + 1$. Moreover, if the UAVs monitoring problem can be approximated with an approximation ratio of $\gamma$, the RMCCP can be approximated with an approximation ratio of $2 \gamma + 1$.
    \end{framedthm}
