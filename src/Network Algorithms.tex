...
\begin{document}
    ...
    \begin{framedlem}[label={blossom lemma}]{Cycle contraction}
        Let $G$ be a graph, let $M$ be a matching of $G$, and let $B$ be a blossom in $G$ w.r.t. $M$ that is \tit{node-disjoint} from $M$. Let $G'$ be the graph obtained by contracting $B$ over its uncovered vertex w.r.t. $M$, and let $M'$ be the matching induced by $M$ on $G'$. Then $M$ is a maximum matching of $G$ if and only if $M'$ is a maximum matching of $G'$.
    \end{framedlem}
