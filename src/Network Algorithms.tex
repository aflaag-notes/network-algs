...
\begin{document}
    ...
    placeholder \todo{missing first part}
    ...
    From this, we can define an algorithm that is able to find a maximum matching on a bipartite graph.
    ...
    \begin{framedalgo}{Maximum matching}
        ...
        \begin{algorithmic}[1]
            \Function{maximumMatchingBipartiteGraphs}{$G$}
                \State $M := \varnothing$
    ...
    \idea{
        The idea of the algorithm follows directly from \cref{aug paths}, since when the algorithm returns $M$, there are no more augmenting paths w.r.t. $M$ in $G$, therefore $M$ is a maximum matching.
    }
