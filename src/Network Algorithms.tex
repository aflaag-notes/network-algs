...
\begin{document}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item clearly, this is a TS tour on $K_n^O$, since $O$ is derived from $T$, which is a spanning tree, therefore it will contain all the odd-degree vertices of $K_n$; by triangle inequality, this implies that $w(A) \le w(H^*)$
            \item since $N^*$ is an optimal TS tour on $K_n^O$ by definition, we have that $w(N^*) \le w(A)$
        ...
        \begin{itemize}
            ...
            \item finally, since $w(U) = w(M) + w(T)$, given a solution of the algorithm $H$, we have that $$w(H) \le w(U) = w(M) + w(T) \le \dfrac{w(H^*)}{2} + w(H^*) \le \dfrac{3}{2}w(H^*)$$ where $w(H) \le w(U)$ by definition of $H$, and $w(T) \le w(H^*)$ is a consequence of \cref{tsp bound}
        \end{itemize}
    \end{proof}
    ...
    \proofiff{
        ...
        \begin{itemize}
            ...
            \item Consider the case in which $P$ crosses $b$. Note that any edge of $E(G) - E(B)$ that has an endpoint on $B$ cannot be in $M$, because 
                ...
                \begin{itemize}
                    \item the uncovered vertex of $B$ is \tit{node-disjoint} from $M$, by hypothesis
        ...
        Let $B$ be a blossom in $G$ w.r.t. $M$ that is \tit{node-disjoint} from $M$, and let $b$ be the node representing $B$ in $G$. Thus, as before, in $G$ two cases may occur.
