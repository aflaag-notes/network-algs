...
\begin{document}
    ...
    Usually the minimum color used for the labeling is 0, thus an $L(h, k)$-labeling having span (i.e. bandwidth) $\sigma_{h, k}(G)$ uses $\sigma_{h, k}(G) + 1$ different colors, which is counterintuitive but this notation is used for historical reasons).
    ...
    It has been proven that both the $L(0,1)$-labeling of planar graphs, and the $L(1,1)$-labeling of general, planar, bounded degree and unit-disk graphs are all \NPComplete problems. Now, consider the following special form of the $L(2,1)$-labeling problem.
    ...
    We will now cover \tit{exact results} regarding $\lambda_{2,1}$. Consider a \tit{clique graph} $K_n$; since all the nodes of a clique graph are pairwise adjacent, it must be that $$\lambda_{2, 1}(K_n) = 2(n - 1)$$. 

    Differently, consider a \href{https://en.wikipedia.org/wiki/Star_(graph_theory)}{\tit{star graph}} $K_{1, t}$; we can prove that $$\lambda_{2,1}(K_{1,t}) = t + 1$$ easily:
    ...
    \begin{framedthm}{$\lambda_{2,1}$ on trees}
    ...
    \begin{proof}
        ...
        Inductively, assume that we have already labeled all the nodes from $v_1$ to $v_i$, and consider $v_{i + 1}$ which has to be labeled. Let $v_j$ be $v_i + 1$ parent w.r.t. $T_n$, and clearly $j \le i + 1$; note that, by definition of $\Delta$, $v_j$ has at most $\Delta - 1$ adjacent neighbours \todo{snapshot ciao armando}
    \end{proof}

    In the original paper, it was conjectured that decide whether the correct value for a given $T_n$ tree is $\Delta + 1$ or $\Delta + 2$ is \NPComplete, but this conjecture was later disproved by \textcite{chang} in 1996, which provided a polynomial $O(\Delta^{4.5} n)$ algorithm based on a \tit{dynamic programming} approach. Multiple authors have proposed many various algorithms attempting to improve this time complexity, and finally in 2008 \textcite{hanusama} proposed a linear algorithm.
