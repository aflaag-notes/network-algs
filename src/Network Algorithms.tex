...
\begin{document}
    ...
    \begin{framedobs}[label={acyclic min-CDSs}]{Acyclic min-CDSs}
        Given a graph $G$, and a min-CDS $D$, by way of contradiction, assume that $D$ contains a cycle; then, removing one vertex from $D$'s cycle would still yield a CDS, of lower cardinality, therefore $D$ was not a min-CDS. This proves that any min-CDS must be acyclic.
    \end{framedobs}
    ...
    \begin{proof}
        Consider a min-CDS $D$ for $G$; a ST $T'$ for $G$ can be constructed by starting from $D$, by simply considering $D$ itself with the nodes in $V(G) - D$ as leaves of $T'$. Moreover, $T'$ spans $G$ completely, because $D$ is a DS, which means that all the nodes in $V(G) - D$ are covered by definition of $D$. This proves that $T'$ is both connected, acyclic (because of \cref{acyclic min-CDSs}), and spans $G$ completely, therefore it is a spanning tree of $G$, and because $T$ is a max-leaf ST for $G$, it follows that $$l \ge l' = \abs{V(G)} - \abs D = n - d$$ where $l'$ is the number of leaves of $T'$, which is $n - d$ by construction.
