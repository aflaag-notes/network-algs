...
\begin{document}
    ...
    The primary goal is to \tbf{minimize energy consumption}, as all devices rely on a common electricity source (e.g., when the stations are deployed in an ad-hoc fashion and connected to a centralized power supply). To achieve this goal, the stations collaborate to provide specific \tbf{connectivity properties}, by \tit{dynamically adjusting} their transmission ranges. Depending on the desired connectivity property, various optimization problems arise, each with unique challenges.

    In particular, the \tbf{transmission graph} is required to
    ...
    For what concerns the last requirement, given a source node $s$, let the \tbf{Broadcast Range Assignment} (\tit{Broadcast}, for short) a range assignment that induces a communication graph $G$ containing a directed spanning tree rooted at $s$. We define the following problem.

    \begin{frameddefn}{Min Broadcast problem}
        Given a set of nodes, a source node $s$, determine the Broadcast that minimizes the overall energy consumption.
    \end{frameddefn}
    
    Note that we are assuming that the energy required to perform a transmission between two nodes is represented by the \tit{weight} between the two nodes.

    Now, consider the following computational problem.

    \begin{frameddefn}[label={set cover}]{Set cover}
    ...
    \begin{framedthm}{Inapproximability of Set Cover}
    ...
    \begin{proof}
        ...
        Let $C'$ be a solution for the instance of the set cover problem we considered; such a solution a
    \end{proof}
    ...
    Voronoi diagrams can be computed by repeatedly intersecting halfplanes, as shown in the pictures below:
    ...
    Consider a sweep line sweeping the given points in $\mathcal P$. By definition, each point in $\mathcal P$ will describe a parabola along with the sweep line that continuously goes down
    ...
    If a point is above the beach line, it must be closer to one of the points in $\mathcal P$ above the sweep line than to the sweep line itself, by definition. Therefore, such a point will lie inside the Voronoi cell of the point in $\mathcal P$ that has already been swept by $l$, implying that the Voronoi diagram above the beach line is completely determined.
